...
export async function GET({ url }) {
	...
	try {
		// Edge Function 호출
		const edgeFunctionUrl = `${EDGE_FUNCTION_URL}?endpoint=location&locationKey=${locationKey}`;
		...
		if (!response.ok) {
			throw error(response.status, `Location info fetch failed: ${response.status}`);
		}
		...
		throw error(500, 'Failed to get location info');
	}
}